...
describe("CLI E2E Test", { timeout: 40_000 }, () => {
  ...
  describe("CLI E2E Test", { timeout: 60_000 }, () => {
    ...
    it("should create a new project with Git initialized and initial commit", async () => {
      const serviceName = `my-test-project-git-${Date.now()}`;
      ...
      const inputs = [
        ...
        "y", // initGit (default is true, so just hitting enter would also work)
      ];
      ...
      const { stdout: commitMessage } = await execa(
        "git",
        ["log", "-1", "--pretty=%B"],
        { cwd: projectPath },
      );
      expect(commitMessage.trim()).toBe("initial commit");
    ...
    it("should create a new project without Git initialized", async () => {
      const serviceName = `my-test-project-no-git-${Date.now()}`;
      const projectPath = path.join(tempDir, serviceName);
      ...
      // Check if Git was NOT initialized
      expect(fs.existsSync(path.join(projectPath, ".git"))).toBe(false);

      // Check if template placeholders were updated correctly (same as above)
      const pkgJson = await fs.readJson(path.join(projectPath, "package.json"));
      expect(pkgJson.name).toBe(serviceName);
